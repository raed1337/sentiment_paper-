...
df  = read_data("DatasetLinAppReviews_test_*")
...
for j in range(4):
    ...
    for i in range(kf):
        test_df  = df[df.index == "DatasetLinAppReviews_test_{}".format(i)]
        train_df = df[df.index != "DatasetLinAppReviews_test_{}".format(i)]
...
df_final.to_excel('DatasetLinAppReviews.xlsx',index=False)
