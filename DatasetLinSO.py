...
df  = read_data("DatasetLinSO_test_*")
...
for j in range(4):
    ...
    for i in range(kf):
        test_df  = df[df.index == "DatasetLinSO_test_{}".format(i)]
        train_df = df[df.index != "DatasetLinSO_test_{}".format(i)]
...
df_final.to_excel('DatasetLinSO.xlsx',index=False)
