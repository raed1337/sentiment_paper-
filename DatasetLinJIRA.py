...
df  = read_data("DatasetLinJIRA_Cleaned_test_*")
...
for j in range(4):
    ...
    for i in range(kf):
        test_df  = df[df.index == "DatasetLinJIRA_Cleaned_test_{}".format(i)]
        train_df = df[df.index != "DatasetLinJIRA_Cleaned_test_{}".format(i)]
...
df_final.to_excel('DatasetLinJIRA.xlsx',index=False)
